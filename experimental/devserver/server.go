...
func shouldAllowCors(request *http.Request) bool {
	...
	return host == hostname || host == "localhost" || strings.HasPrefix(host, "local.")
}
...
func CreateFileHandler(servingPath string, pkgs []string, base string) http.HandlerFunc {
	pkgPaths := chainedDir{}
	...
	defaultPage := []byte(fmt.Sprintf(`<!doctype html>
		<html>
			<head>
				<title>bazel js_devserver</title>
			</head>
			<body>
				<script src="%s"></script>
			</body>
